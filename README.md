...
- `（1）可维护性` 设计模式有助于降低模块间的耦合度。对代码重构和换用不同的场景更加容易，使程序员在大型团队中的工作以及与其他程序员合作变得更加容易。
- `（2）沟通` 设计模式为处理不同类型的对象提供一套通用的术语。程序员因此可以更加简明的描述自己的系统的工作方式。
- `（3）性能` 某些设计模式是会优化运行速度，但是某些模式需要为可维护性付出代价，损失性能。
